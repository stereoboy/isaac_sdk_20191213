...
void NodeGroup::start() {
  for (const std::string& node_name : get_node_names()) {
    Node* child = node()->app()->findNodeByName(node_name);
    if (child == nullptr) {
...
Node& NodeGroup::getNodeByIndex(size_t index) {
  assertChildAccess();
  ASSERT(index < nodes_.size(), "Index out of range: %zd >= %zd", index, nodes_.size());
  return *nodes_[index];
}

Node& NodeGroup::getNodeByName(const std::string& name) {
  assertChildAccess();
...
Node* NodeGroup::findNodeByName(const std::string& name) {
  assertChildAccess();

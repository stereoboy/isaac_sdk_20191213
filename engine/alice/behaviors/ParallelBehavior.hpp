...
class ParallelBehavior : public Behavior {
 public:
  ...
  // If the number of succeeding children is larger than this threshold the parallel behavior will
  // succeed. If this is set to -1 it means that all children must succeed. Value must be -1 or
  // larger. By default all children must succeed.
...
ISAAC_ALICE_REGISTER_CODELET(isaac::alice::behaviors::ParallelBehavior);

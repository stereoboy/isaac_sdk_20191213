...
class Renderer2D extends SightRenderer {
  ...
  render(op, time) {
    ...
    if (op.p) {
      ...
      if (op.p.t === "c") {
        ...
        const mat = SightChannel.GetTransform(
  ...
  static GetPose2(matrix, projection) {
    if (Renderer2D.IsPerspectiveProjection(projection)) {
      const pt = new THREE.Vector4(0, 0, 0, 1).applyMatrix4(matrix).applyMatrix4(projection);
      return {x: pt.y / pt.w, y: pt.x / pt.w, angle: 0.0};
    }

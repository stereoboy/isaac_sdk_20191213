...
class VerifyConfusionMatrices(Codelet):
    ...
    def start(self):
        ...
        self.set_isaac_param("results_directory", "/tmp/object_detection_evaluation/")
        self.set_isaac_param("class_names", ["Ball", "Bowling_Pin", "Chair", "Plants", "Trashcan"])
        self.set_isaac_param("min_precision", 0.0)
        self.set_isaac_param("min_recall", 0.0)
        self.set_isaac_param("visualization_interval", 2)

        try:
            self.num_images = 0
            self.iou_thresholds = []
            self.confusion_matrics = None
            self.class_names = self.get_isaac_param("class_names")

            # Create a folder to store all the generated files (visualizations and summary JSON report)
            metric_plots_dir = self.get_isaac_param("results_directory")
    ...
    def tick(self):
        try:
            odmp_msg = self.rx.get_proto()
            buffer_data = self.rx.get_buffer_content(odmp_msg.confusionMatrices.dataBufferIndex)
            ...
            if self.get_tick_count() % self.get_isaac_param("visualization_interval") == 0:
        ...
        try:
            ...
            for iou_idx in range(len(self.iou_thresholds)):
                for class_idx in range(len(self.class_names)):
                    assert self.per_class_precisions[class_idx, iou_idx] >= self.get_isaac_param(
                        "min_precision"), "Precision is too low for class {} with IoU {}".format(
                            self.class_names[class_idx], self.iou_thresholds[iou_idx])
                    assert self.per_class_recalls[class_idx, iou_idx] >= self.get_isaac_param(
                        "min_recall"), "Recall is too low for class {} with IoU {}".format(
                            self.class_names[class_idx], self.iou_thresholds[iou_idx])
    ...
    def create_json_report(self):
        ...
        with open(self.get_isaac_param("results_directory") + "object_detection_metrics.json",
                  'w') as f:
    ...
    def plot_confusion_matrix(self, conf_mat, iou_threshold):
        ...
        plt.tight_layout()
        plt.savefig(
            str(self.get_isaac_param("results_directory")) + "{:.2f}".format(iou_threshold) +
            "-confmat.png")
        plt.cla()
    ...
    def plot_precision_recall(self, class_name, per_class_precisions, per_class_recalls, auc):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(str(self.get_isaac_param("results_directory")) + class_name + "-PR-curve.png")
        plt.cla()


def main():
    node_pycodelet_map = {"object_detection_metrics_verifier": VerifyConfusionMatrices}
    app = Application(
        "packages/ml/apps/evaluate_object_detection/evaluate_object_detection_log.app.json")
    app.register(node_pycodelet_map)
    app.start_wait_stop(100)

...
*/
#include "TensorListGenerator.hpp"
...
void TensorListGenerator::start() {
  tickPeriodically();
...
void TensorListGenerator::tick() {
  auto proto_tensors = tx_sample().initProto().initTensors(get_num_tensors());
  for (size_t i = 0; i < proto_tensors.size(); i++) {
    switch (get_element_type()) {
      case TensorListGeneratorElementType::kFloat32: {
        Tensor3f out(buffer_float32_.dimensions());
        Copy(buffer_float32_, out);
        ToProto(std::move(out), proto_tensors[i], tx_sample().buffers());
      } break;
      case TensorListGeneratorElementType::kInt32: {

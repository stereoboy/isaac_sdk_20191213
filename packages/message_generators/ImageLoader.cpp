...
void ImageLoader::createAndPublishColorImageMessage(Image3ub image, const int64_t acqtime) {
  auto rbg_proto = tx_color().initProto();

  // Pinhole for color camera
  auto pinhole = rbg_proto.initPinhole();
  ...
  ToProto(std::move(image), rbg_proto.initImage(), tx_color().buffers());
  rbg_proto.setColorSpace(ColorCameraProto::ColorSpace::RGB);

  // Distortion parameters for color camera
  auto distortion = rbg_proto.initDistortion();
  distortion.setModel(DistortionProto::DistortionModel::BROWN);
  ToProto(get_distortion_coefficients().cast<float>(), distortion.getCoefficients());

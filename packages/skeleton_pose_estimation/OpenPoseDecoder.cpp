...
void OpenPoseDecoder::start() {
  ...
  tickOnMessage(rx_tensors());
}

void OpenPoseDecoder::tick() {
  // for estimating key points
  auto tensor_reader = rx_tensors().getProto().getTensors();
  int64_t acqtime = rx_tensors().acqtime();
  ...
  TensorConstView3f part_affinity_fields, gaussian_heatmap, maxpool_heatmap;
  if (!FromProto(tensor_reader[0], rx_tensors().buffers(), part_affinity_fields) ||
      !FromProto(tensor_reader[1], rx_tensors().buffers(), gaussian_heatmap) ||
      !FromProto(tensor_reader[2], rx_tensors().buffers(), maxpool_heatmap)) {
    reportFailure("Unable to read from rx_tensors (part_affinity_fields, ...).");
    return;
  }
